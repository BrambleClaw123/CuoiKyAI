    ...
    def backtrack(self, assignment, csp):
        ...
        for value in self.order_domain_values(var, assignment, csp):
            if self.is_consistent(var, value, assignment, csp):
                ...
                removed = self.forward_check(var, value, assignment, csp)
                if removed is not None:
                    result = self.backtrack(assignment, csp)
                    if result is not None:
                        return result
                self.restore_domains(removed, csp)
                del assignment[var]
                self.explored_states.append(copy.deepcopy(assignment))
        return None
    ...
    def forward_check(self, var, value, assignment, csp):
        ...
        for other_var in csp['variables']:
            if other_var == var or other_var in assignment:
                continue
            ov_size = board.vehicles[other_var].size
            ov_orient = board.vehicles[other_var].orientation
            for other_value in csp['domains'][other_var][:]:
                or_, oc = other_value
                other_cells = set()
                for i in range(ov_size):
                    if ov_orient == 'H':
                        other_cells.add((or_, oc + i))
                    else:
                        other_cells.add((or_ + i, oc))

                if not cells.isdisjoint(other_cells):
                    csp['domains'][other_var].remove(other_value)
                    removed.append((other_var, other_value))

            if len(csp['domains'][other_var]) == 0:
                self.restore_domains(removed, csp)
                return None 
        return removed

    def restore_domains(self, removed, csp):
        for other_var, value in removed:
            csp['domains'][other_var].append(value)